...
#define for_each_supported_arrays_rcd(rcd, qac) \
    __for_each_qac_entry(rcd, qac, struct ipr_supported_arrays) if (rcd->common.record_id == IPR_RECORD_ID_SUPPORTED_ARRAYS)

#define for_each_qac_entry(rcd, qac) \
    __for_each_qac_entry(rcd, qac, struct ipr_common_record)

#define for_each_dev_rcd(rcd, qac) \
    __for_each_qac_entry(rcd, qac, struct ipr_dev_record) if (ipr_is_device_record(rcd->common.record_id))

#define for_each_array_rcd(rcd, qac) \
    __for_each_qac_entry(rcd, qac, struct ipr_array_record) if (ipr_is_array_record(rcd->common.record_id))
...
#define for_each_primary_ioa(ioa) \
    __for_each_ioa(ioa, ipr_ioa_head) if (!ioa->is_secondary)

#define for_each_secondary_ioa(ioa) \
    __for_each_ioa(ioa, ipr_ioa_head) if (ioa->is_secondary)

#define for_each_sas_ioa(ioa) \
    __for_each_ioa(ioa, ipr_ioa_head) if (!__ioa_is_spi(ioa))
...
#define for_each_hotplug_dev(i, d) \
    for_each_dev(i, d) if (ipr_is_af_dasd_device(d) || ipr_is_gscsi(d))

#define for_each_af_dasd(i, d) \
    for_each_dev(i, d) if (ipr_is_af_dasd_device(d))

#define for_each_dev_in_vset(v, d) \
    for_each_af_dasd((v)->ioa, d) if (ipr_is_array_member(d) && d->array_id == (v)->array_id)

#define for_each_vset(i, d) \
    for_each_dev(i, d) if (ipr_is_volume_set(d) && !d->array_rcd->start_cand)

#define for_each_array(i, d) \
    for_each_dev(i, d) if (ipr_is_array(d) && !d->array_rcd->start_cand)

#define for_each_dvd_tape(i, d)                                                       \
    for_each_dev(i, d) if (d->scsi_dev_data && (d->scsi_dev_data->type == TYPE_ROM || \
                                                d->scsi_dev_data->type == TYPE_TAPE))

#define for_each_ses(i, d) \
    for_each_dev(i, d) if (d->scsi_dev_data && d->scsi_dev_data->type == TYPE_ENCLOSURE)

#define for_each_hot_spare(i, d) \
    for_each_dev(i, d) if (ipr_is_hot_spare(d))

#define for_each_disk(i, d) \
    for_each_dev(i, d) if ((d)->scsi_dev_data && (ipr_is_af_dasd_device(d) || ipr_is_gscsi(d)))

#define __for_each_disk(i, d)                                                              \
    for_each_dev(i, d) if ((d)->scsi_dev_data && ((d)->scsi_dev_data->type == TYPE_DISK || \
                                                  (d)->scsi_dev_data->type == IPR_TYPE_AF_DISK))

#define for_each_standalone_disk(i, d) \
    for_each_disk(i, d) if (!ipr_is_hot_spare(d) && !ipr_is_array_member(d))

#define __for_each_standalone_disk(i, d)                                       \
    __for_each_disk(i, d) if (!ipr_is_hot_spare(d) && !ipr_is_volume_set(d) && \
                              !ipr_is_array_member(d))

#define for_each_standalone_af_disk(i, d) \
    for_each_standalone_disk(i, d) if (ipr_is_af_dasd_device(d))

#define for_each_jbod_disk(i, d) \
    for_each_disk(i, d) if (ipr_is_gscsi(d))
...
#define for_each_bus_attr(ptr, page28, i)                                  \
    for (i = 0, ptr = page28->attr; i < page_28->page_hdr.num_dev_entries; \
         i++, ptr = (struct ipr_mode_page_28_scsi_dev_bus_attr *)((u8 *)ptr + page_28->page_hdr.dev_entry_length))
...
struct unsupported_af_dasd
{
    ...
    unsigned int supported_with_min_ucode_level;
...
#define for_each_phy(phy, port)                      \
    for (phy = (port)->phy;                          \
         ...
         (phy < ((port)->phy + IPR_IOA_MAX_PORTS));  \
         phy++)
...
#define for_each_elem_status(elem, sd, sc)                                                            \
    for (elem = &((sd)->elem_status[ipr_dev_elem_offset(sc)]);                                        \
         ...
         elem < &((sd)->elem_status[IPR_NUM_DRIVE_ELEM_STATUS_ENTRIES]);                              \
         elem++)
...
int ipr_mode_select(int, void *, int);
